...
logger = logging.getLogger("nexaflow.errors")
...
def install_error_handlers(app: FastAPI) -> None:
    ...
    async def _api_error(_: Request, exc: APIError) -> JSONResponse:
        return JSONResponse(
            error_body(exc.code, exc.message), exc.status_code, headers=exc.headers
        )
    ...
    async def _validation_error(_: Request, exc: RequestValidationError) -> JSONResponse:
        ...
        return JSONResponse(
            error_body("validation_error", "The request failed validation.", fields=fields),
            422,
        )

    @app.exception_handler(Exception)
    async def _unexpected(request: Request, exc: Exception) -> JSONResponse:
        # Logged in full server side; one generic line goes to the client.

...
Every action resolves its destination through :func:`validate_target` before a
request is made, so a workflow cannot be aimed at the host's metadata service
or a database on the private network.
"""
    ...
    for address in _resolve(parsed.hostname):
        if (
            ...
            raise UnsafeTargetError(
                ...
                "workflows may only call public hosts"
            )
    return url

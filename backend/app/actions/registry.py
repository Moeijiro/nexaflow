...
ACTIONS: dict[str, Action] = {
    action.type: action for action in (DiscordAction(), TelegramAction(), HttpAction())
}
...
def describe_actions() -> list[dict[str, Any]]:
    """Powers the integrations page and the builder, straight from the code."""
    return [
        {
            "type": action.type,
            "label": action.label,
...
def redact_config(action_type: str, stored: dict[str, Any]) -> dict[str, Any]:
    """What the API may show: secrets replaced, never decrypted."""
    action = get_action(action_type)
    data: dict[str, Any] = {}

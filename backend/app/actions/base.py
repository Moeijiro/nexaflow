...
Adding an integration is one module here plus one line in the registry: a
Pydantic config model, the fields that are secret, and a ``run`` method.
"""
...
class ActionOutcome:
    ...
    # Whether a retry could plausibly help (timeouts, 429, 5xx — not 400).
...
class Action(ABC):
    ...
    #: Shown on the integrations page.

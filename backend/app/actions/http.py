"""Call any HTTP API with a templated body."""
...
BODYLESS = {"GET", "DELETE"}
# Headers the client controls; a workflow setting them breaks the request or
# the SSRF check.
...
class HttpAction(Action):
    ...
    async def run(self, config: HttpConfig, context: ActionContext) -> ActionOutcome:
        ...
        if config.body_template:
            rendered = render(config.body_template, context.payload, json_string=config.json_body)
        ...
        try:
            async with build_client() as client:
                response = await client.request(config.method, url, headers=headers, content=content)
        ...
        return ActionOutcome(
            ...
            retryable=not ok and (response.status_code == 429 or response.status_code >= 500),
